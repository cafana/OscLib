...
namespace osc {
  ...
  void _OscCalcNuFast<T>::SetTh12(const T& th12) {
    ...
    T s12 = sin(th12); this->fPre.s12sq = s12*s12; // Fast square that doesn't need to compute sine twice.
  }
  ...
  void _OscCalcNuFast<T>::SetTh13(const T& th13) {
    this->fDirty = true;
    this->fTh13 = th13;
    T s13 = sin(th13); this->fPre.s13sq = s13*s13;
  }
  ...
  void _OscCalcNuFast<T>::SetTh23(const T& th23) {
    this->fDirty = true;
    this->fTh23 = th23;
    T s23 = sin(th23); this->fPre.s23sq = s23*s23;
  }
  ...
  template<typename T>
  T _OscCalcNuFast<T>::P(int flavBefore, int flavAfter, double E) {
  ...
  void _OscCalcNuFast<T>::RecomputeEnergyIndependentTerms(void) {
    //#define Ut2sq this->fPre.s13sq * this->fPre.s12sq * this->fPre.s23sq
    //#define Um2sq (1-this->fPre.s12sq) * (1-this->fPre.s23sq)
    //this->fPre.Jrr = sqrt( Um2sq * Ut2sq );
  ...
  template<typename T>
  void _OscCalcNuFast<T>::RecomputeProbabilityMatrix(double E, bool isAnti) {
